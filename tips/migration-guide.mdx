...
| **Hosting** | Lovable Cloud | Kleap Hosting (Cloudflare edge) |
| **Database** | Supabase | Kleap Database (built-in, opt-in) |
...
- **Kleap Database** — a built-in Postgres database with Row Level Security and
  built-in email/password user accounts. It's opt-in and added on demand, so
  simple sites stay purely static.
