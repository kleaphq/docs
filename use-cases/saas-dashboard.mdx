...
Dynamic data and user accounts use **Kleap Database** — a managed Postgres database built into Kleap (with built-in email/password accounts and Row Level Security). It's opt-in: enable it when a step needs it. You don't connect your own database.
...
Enable **Kleap Database** for this app. It's a managed Postgres database with built-in email/password accounts, so you can link data to the signed-in user and protect it with Row Level Security — no external database to connect.
