...
Need logins, saved data, or content that changes per user? Turn on the **Kleap Database** — a managed Postgres database with built-in user accounts. It's optional and available on demand; just ask the AI to add it when your app needs it. You don't connect or manage any outside database yourself.
...
Data access is protected by **Row-Level Security**, so each signed-in user can only see and change **their own rows**. This is the primary defense for your data: even though many users share one database, RLS keeps everyone's data separated at the database level.

<AccordionGroup>
  <Accordion title="Scope data to the signed-in user">
    RLS ensures a user only ever reads or writes rows that belong to them. When you add data features through Kleap, ask the AI to make sure each table is scoped to the current user so no one can read someone else's records.
...
| **Data leaks between users** | Row-Level Security scopes each user to their own rows |
...
<Steps>
  ...
  <Step title="Data scoped per user">
    If you store data, Row-Level Security limits each user to their own rows.
  </Step>
